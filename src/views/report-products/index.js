  ...
  const [columns, setColumns] = useState([
    {
      ...
      is_show: true,
      render: (stock, data) => {
        const { id, translation } = stock?.product || {};
        return (
          <Link to={`/report/products?product_id=${id}`}>
            {translation?.title}
          </Link>
        );
      },
    ...
    },
    {
      ...
      render: (stock, row) => {
        ...
        return (
          <Link to={`/report/products?category_id=${category?.id}`}>
    ...
    },
    {
      ...
      dataIndex: 'stock',
      render: (stock, data) =>
        stock?.stock_extras?.map((item) => {
          if (item.group.type === 'color') {
            return <Badge color={item.value.value} title={item.value.value} />;
          } else return <Tag>{item.value.value}</Tag>;
        }),
      key: 'Extras name',
      is_show: true,
    },
