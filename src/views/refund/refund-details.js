...
export default function OrderDetails() {
  ...
  const products = data?.details?.flatMap((detail) =>
    detail?.products?.map((product) => product),
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) =>
        numberToPrice(
          row?.origin_price / (row?.quantity ?? 1),
    ...
    },
    {
      ...
      render: (_, row) =>
        numberToPrice(
          row?.rate_discount / (row?.quantity ?? 1),
          defaultCurrency?.symbol,
        ),
    },
    {
                  ...
                  <>
                    <Space className='w-100'>
                      <Avatar
                        shape='square'
                        size={64}
                        src={IMG_URL + shop?.logo_img}
                      />
                      <div>
                        <h5>{shop?.translation.title}</h5>
                        <div className='delivery-info'>
                          <b>
                            <BsFillTelephoneFill />
                          </b>
